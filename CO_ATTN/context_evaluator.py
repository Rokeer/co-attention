...
class Evaluator():
    ...
    def __init__(self, prompt_id, use_char, out_dir, modelname, train_x, dev_x, test_x, train_c, dev_c, test_c, train_y, dev_y, test_y, train_context, dev_context, test_context, char_only=False):
    ...
    def evaluate(self, model, epoch, batch_size=32, print_info=False):
        if self.use_char:
            ...
            train_pred = model.predict([self.train_x, self.train_context], batch_size=batch_size).squeeze()
            dev_pred = model.predict([self.dev_x, self.dev_context], batch_size=batch_size).squeeze()
            test_pred = model.predict([self.test_x, self.test_context], batch_size=batch_size).squeeze()
        ...
        if self.dev_qwk > self.best_dev[0]:
            ...
            with open(self.out_dir + '/' + self.modelname + 'output_test_score.txt', 'w') as file:
